...
\label{eq:bce}
\end{equation}
where $y_i$ is the true label and $\hat{y}_i$ is the predicted probability. We implement early stopping with patience of 10 rounds to prevent overfitting, monitoring validation loss during training. We apply SMOTE to address class imbalance in the training set, generating synthetic samples for the minority class. Importantly, SMOTE is applied only to training data after the temporal split, ensuring test data remains 100\% original. Our analysis of SMOTE-generated samples confirms they create true interpolations rather than copies: across all assets, 0\% of synthetic samples are exact duplicates, and over 93\% have meaningful Euclidean distance ($>$0.1 in normalized space) from original samples. All experiments use random seed 42 for reproducibility.

\textbf{Evaluation Metrics:} We evaluate model performance using accuracy, precision, recall, and F1 score. Accuracy is defined in Equation~\ref{eq:accuracy}:
\begin{equation}
\text{Accuracy} = \frac{TP + TN}{TP + TN + FP + FN}
\label{eq:accuracy}
\end{equation}
where TP, TN, FP, and FN represent true positives, true negatives, false positives, and false negatives respectively. We also compute precision, recall, and F1 as defined in Equations~\ref{eq:precision}--\ref{eq:f1}:
\begin{equation}
...
\label{eq:f1}
\end{equation}
These metrics are critical for imbalanced datasets where accuracy alone can be misleading.
...
To address the limitations of accuracy-only evaluation, Table~\ref{tab:comprehensive} presents comprehensive metrics for representative assets. These results reveal important insights about class imbalance: while accuracy may appear high (e.g., AAPL at 68.09\%), some models predict only the majority class, resulting in 0\% precision/recall/F1 for the minority class. BTC-USD achieves the best minority class detection with 42.55\% F1 score.

\begin{table}[htbp]
\caption{Comprehensive Metrics (Test Set, RandomForest)}
\begin{center}
\begin{tabular}{|l|c|c|c|c|}
\hline
\textbf{Asset} & \textbf{Accuracy} & \textbf{Precision} & \textbf{Recall} & \textbf{F1} \\
\hline
AAPL & 68.09\% & 0.00\% & 0.00\% & 0.00\% \\
\hline
MSFT & 80.85\% & 30.77\% & 18.18\% & 22.86\% \\
\hline
GOOGL & 47.52\% & 47.06\% & 10.96\% & 17.78\% \\
\hline
BTC-USD & 74.29\% & 55.56\% & 34.48\% & 42.55\% \\
\hline
ETH-USD & 61.90\% & 55.00\% & 26.19\% & 35.48\% \\
\hline
\end{tabular}
...
We trained models with and without SMOTE to quantify its effect. Table~\ref{tab:smote} shows that SMOTE provides mixed, asset-dependent results: it generally improves F1 score (+5--7\%) by increasing recall, but may slightly reduce accuracy ($-$2 to $-$4\%) due to increased false positives.

\begin{table}[htbp]
\caption{SMOTE Impact on Performance (Average Across Models)}
\begin{center}
\begin{tabular}{|l|c|c|c|c|}
\hline
\textbf{Asset} & \textbf{$\Delta$Acc} & \textbf{$\Delta$Prec} & \textbf{$\Delta$Recall} & \textbf{$\Delta$F1} \\
\hline
...
Table~\ref{tab:feature_ablation} presents asset-specific feature ablation results, showing that feature importance varies by asset type. MSFT shows the strongest dependence on momentum features (+7.09\% accuracy drop when removed), while ETH-USD depends more on trend features (+1.43\% drop). These findings suggest that optimal feature selection should be asset-specific.

\begin{table}[htbp]
\caption{Feature Ablation: Accuracy Drop When Removing Feature Groups}
\begin{center}
\begin{tabular}{|l|c|c|c|c|}
\hline
\textbf{Asset} & \textbf{w/o Mom.} & \textbf{w/o Trend} & \textbf{w/o Vol.} & \textbf{w/o Volume} \\
\hline
AAPL & 0.00\% & $-$0.71\% & 0.00\% & 0.00\% \\
\hline
MSFT & \textbf{+7.09\%} & $-$2.84\% & $-$0.71\% & 0.00\% \\
\hline
BTC-USD & +0.95\% & +0.95\% & +0.95\% & $-$2.38\% \\
\hline
ETH-USD & +0.48\% & \textbf{+1.43\%} & $-$0.95\% & 0.00\% \\
\hline
\end{tabular}
...
Regarding SMOTE, our with/without analysis reveals asset-dependent effects: while SMOTE generally improves recall and F1 (+5--7\%) by better detecting minority class signals, it may reduce precision and accuracy for some assets (e.g., MSFT shows $-$1.39\% F1 with SMOTE). This suggests SMOTE benefits should be evaluated per-asset rather than applied uniformly.
