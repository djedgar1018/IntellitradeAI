...
We employ three complementary models in our ML ensemble, each selected for its unique strengths in financial prediction. Long Short-Term Memory (LSTM) networks \cite{b16} excel at capturing temporal dependencies in sequential price data. Random Forest \cite{b25} provides robust ensemble predictions through bagging, effectively handling noisy financial data. XGBoost \cite{b5} offers gradient boosting with built-in regularization to prevent overfitting.

Our LSTM Neural Network uses 2 LSTM layers with 50 units each, dropout of 0.2, and is trained with Adam optimizer. Our Random Forest classifier uses 100 trees with maximum depth of 10 and balanced class weights. Our XGBoost classifier uses 150 estimators with learning rate of 0.1 and maximum depth of 6.
...
\hline
\textbf{Parameter} & \textbf{LSTM} & \textbf{Random Forest} & \textbf{XGBoost} \\
\hline
Architecture & 2x50 units & 100 trees & 150 rounds \\
\hline
Regularization & Dropout 0.2 & Max depth=10 & L1/L2 reg. \\
\hline
Optimizer & Adam & N/A & N/A \\
\hline
Training Time & $\sim$120 sec & $\sim$45 sec & $\sim$60 sec \\
\hline
\end{tabular}
\label{tab:models}
\end{center}
...
We trained all three models using 5-fold cross-validation on our dataset of 904,195 preprocessed records. Each fold contains approximately 180,839 samples, with 144,671 for training and 36,168 for validation. We measure performance using accuracy, precision, recall, F1-score, and AUC-ROC metrics.

Fig.~\ref{fig:loss} presents training and validation loss curves for all three models. We trained the LSTM for 100 epochs with early stopping (patience=10), and it achieved convergence at epoch 52 with final training loss of 0.305 and validation loss of 0.362. The gap between training and validation loss (0.057) indicates mild overfitting that we controlled through dropout regularization. Random Forest, being a non-iterative ensemble method, does not have traditional loss curves; instead, we plot out-of-bag error across tree iterations, showing stabilization at approximately 45 trees. XGBoost demonstrated the fastest convergence, with validation loss stabilizing at round 38 (training loss 0.298, validation loss 0.341).
...
Table~\ref{tab:performance} presents our 5-fold cross-validation results across 50 cryptocurrency and 50 stock symbols. We observe that ensemble accuracy for cryptocurrency (68.2\%) is lower than stocks (71.5\%), which we attribute to higher volatility and market inefficiency in crypto markets. While these accuracies may appear modest compared to classification tasks in other domains, financial prediction is inherently challenging due to market efficiency, where prices already reflect available information. Our results are consistent with the literature, where prediction accuracies of 55-70\% are considered competitive \cite{b17}.
...
\begin{tabular}{|l|c|c|c|c|}
\hline
\textbf{Metric} & \textbf{LSTM} & \textbf{RF} & \textbf{XGB} & \textbf{Ensemble} \\
\hline
Accuracy (Crypto) & 64.5\% & 65.8\% & 67.4\% & 68.2\% \\
\hline
Accuracy (Stocks) & 68.1\% & 69.2\% & 70.8\% & 71.5\% \\
\hline
Precision & 0.661 & 0.673 & 0.691 & 0.702 \\
\hline
Recall & 0.648 & 0.658 & 0.672 & 0.685 \\
\hline
F1-Score & 0.654 & 0.665 & 0.681 & 0.693 \\
\hline
AUC-ROC & 0.698 & 0.712 & 0.728 & 0.741 \\
\hline
\end{tabular}
...
\section{Conclusion}
We presented IntelliTradeAI, a comprehensive AI-powered trading agent demonstrating the effectiveness of multi-source signal fusion for financial market prediction. Our tri-signal architecture achieved 68.2\% accuracy for cryptocurrencies and 71.5\% for stocks, representing a 5.4 percentage point improvement (8.6\% relative) over standalone ML approaches. Key contributions include: (1) a unified cross-market analysis framework supporting 100+ cryptocurrencies and comprehensive stock coverage; (2) explainable AI through SHAP analysis; (3) personalized trading plans based on five-tier risk tolerance assessment; (4) SEC-compliant risk disclosures with e-signature authorization; and (5) an interactive dashboard with real-time predictions.

Our work has several limitations: model performance relies on data quality from third-party APIs; models trained on historical data may underperform during unprecedented market conditions; and real-time prediction introduces 1-3 second latency. Future work includes integration of transformer-based models (FinBERT) for improved sentiment analysis, reinforcement learning for dynamic strategy adaptation, and portfolio optimization using modern portfolio theory.
