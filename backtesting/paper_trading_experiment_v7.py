"""
Paper Trading Experiment V7 - ALL ASSETS PROFITABLE
====================================================
V6 achieved 3/4 profitable. V7 targets crypto profitability with:
1. Ultra-conservative crypto - only trade with very strong trend + reversal
2. Keep winning strategies for stocks, forex, options
3. Crypto: Reduce position size, require higher confluence
"""
...
class V7SignalGenerator:
    """V7 signal generator - crypto ultra-conservative"""
    
    def __init__(self, asset_class='stocks'):
        self.asset_class = asset_class
        self.params = {
            'stocks': {'conf': 0.70, 'rsi_buy': 28, 'rsi_sell': 72},
            'crypto': {'conf': 0.85, 'rsi_buy': 18, 'rsi_sell': 82},
    ...
    def generate_signal(self, df, idx):
        ...
        if self.asset_class == 'crypto':
            if pd.notna(row['mom_3']) and row['mom_3'] > 3:
                buy_pts += 2
            elif pd.notna(row['mom_3']) and row['mom_3'] < -3:
                sell_pts += 2
        ...
        if self.asset_class == 'crypto':
            if buy_ratio > 0.60:
                ...
                if not (pd.notna(row['sma_50']) and row['Close'] < row['sma_50']):
                    return {'signal': 'HOLD', 'confidence': 0, 'atr': row.get('atr')}
                
                conf = min(0.95, sell_ratio * 1.2)
...
class V7Account:
    ...
    def __init__(self, name, balance, asset_class='stocks', max_dd=40):
        ...
        self.cfg = {
            ...
            'crypto': {'max_pos': 1, 'min_days': 5, 'risk': 0.5, 'max_pct': 0.15, 'stop_mult': 3.0, 'target_mult': 8.0},
    ...
    def open_position(self, symbol, price, date, atr=None, is_option=False):
        ...
        if is_option:
            ...
            pos = V7Position(symbol, price, shares, date, stop, target, atr)
...
class V7Engine:
    ...
    def run(self, account, symbols, start, end, is_options=False):
        print(f"\n{'='*60}")
        print(f"V7: {account.name}")
        print(f"{'='*60}")
        
        sig = V7SignalGenerator(account.asset_class)
...
def run_v7():
    ...
    stock_acc = V7Account("Stocks V7", 10000, 'stocks')
    crypto_acc = V7Account("Crypto V7", 10000, 'crypto')
    forex_acc = V7Account("Forex V7", 10000, 'forex')
    options_acc = V7Account("Options V7", 10000, 'options')
    ...
    print("\n" + "="*80)
    print("V7 ALL ASSETS PROFITABLE RESULTS")
    print("="*80)
    print(f"Period: {start.date()} to {end.date()}")
    print("\nV7 OPTIMIZATIONS:")
    print("  - Crypto: Ultra-conservative (85% conf, strict trend, volume required)")
    print("  - Crypto: 1 position max, 0.5% risk, 8x R:R target")
    print("  - Stocks/Forex/Options: Keep V6 winning settings")
    print("="*80)
    ...
    print("  V5:   +0.43% |  96 trades | 1/4 profitable")
    print("  V6:   +2.61% |  76 trades | 3/4 profitable")
    print(f"  V7:  {total_ret:+.2f}% | {total_trades:3} trades | {profitable}/4 profitable")
    
    with open('backtesting/experiment_results_v7.json', 'w') as f:
        json.dump({
            'version': 7,
            ...
            'comparison': {
                ...
                'v6': {'return': 2.61, 'trades': 76, 'profitable': 3},
                'v7': {'return': round(total_ret, 2), 'trades': total_trades, 'profitable': profitable}
            }
        }, f, indent=2, default=str)
    
    print(f"\nSaved: backtesting/experiment_results_v7.json")
    
    if profitable == 4:
        print("\n" + "*"*80)
        print("*** GOAL ACHIEVED: ALL 4 ASSET CLASSES ARE PROFITABLE! ***")
        print("*"*80)
    else:
