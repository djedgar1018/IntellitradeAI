"""
Paper Trading Experiment V9 - 12-MONTH ENHANCED PERFORMANCE
============================================================
Extended to 12-month period with optimized parameters:
- Stocks: More aggressive entries, tighter trailing stops
- Forex: Higher frequency, better trend following
- Crypto: Moderate mean reversion with trend confirmation
- Options: Keep V8 winning settings
"""
...
class V9SignalGenerator:
    """V9 signal generator - optimized for higher performance"""
    
    def __init__(self, asset_class='stocks'):
        self.asset_class = asset_class
        self.params = {
            'stocks': {'conf': 0.65, 'rsi_buy': 30, 'rsi_sell': 70},
            'crypto': {'conf': 0.72, 'rsi_buy': 22, 'rsi_sell': 78},
            'forex': {'conf': 0.60, 'rsi_buy': 32, 'rsi_sell': 68},
    ...
    def calculate_indicators(self, df):
        ...
        df['mom_pct'] = df['Close'].pct_change(periods=10) * 100
    ...
    def generate_signal(self, df, idx):
        ...
        strong_uptrend = in_uptrend and short_uptrend and (pd.notna(row['sma_100']) and row['sma_50'] > row['sma_100'])
        
        vol_ok = pd.notna(row['vol_ratio']) and row['vol_ratio'] > 1.1
        ...
        if buy_ratio > 0.52:
            conf = min(0.95, buy_ratio * 1.05)
            
            if in_uptrend:
                conf = min(0.98, conf * 1.15)
                reasons.append("trend+")
            elif short_uptrend and buy_ratio >= 0.65:
...
class V9Position:
    ...
    def update(self, current):
        ...
        if self.trailing and self.atr and pct >= 2.5:
            trail = self.highest - (self.atr * 1.2)
            if trail > self.stop:
                self.stop = trail
...
class V9Account:
    ...
    def __init__(self, name, balance, asset_class='stocks', max_dd=40):
        ...
        self.cfg = {
            'stocks': {'max_pos': 4, 'min_days': 2, 'risk': 1.5, 'max_pct': 0.22, 'stop_mult': 1.6, 'target_mult': 4.5},
            'crypto': {'max_pos': 2, 'min_days': 3, 'risk': 1.0, 'max_pct': 0.18, 'stop_mult': 2.2, 'target_mult': 5.5},
            'forex': {'max_pos': 4, 'min_days': 1, 'risk': 1.8, 'max_pct': 0.28, 'stop_mult': 1.4, 'target_mult': 3.2},
    ...
    def open_position(self, symbol, price, date, atr=None, is_option=False):
        ...
        if is_option:
            ...
            pos = V9Position(symbol, price, shares, date, stop, target, atr)
...
class V9Engine:
    ...
    def _sim(self, symbol, start, end):
        ...
        vol = 0.007 if '=' in symbol else 0.020 if '-USD' in symbol else 0.013
        
        np.random.seed(hash(symbol) % 2**32)
        
        cycle = np.sin(np.linspace(0, 6 * np.pi, len(dates))) * 0.10
        trend_dir = 1 if np.random.random() > 0.35 else -1
        trend = np.linspace(0, 0.15 * trend_dir, len(dates))
        noise = np.random.normal(0, vol, len(dates))
        
        rets = noise + (cycle / len(dates) * 25) + (trend / len(dates) * 8)
        
        prices = base * np.cumprod(1 + rets)
        rng = prices * np.random.uniform(0.008, 0.020, len(dates))
    ...
    def run(self, account, symbols, start, end, is_options=False):
        print(f"\n{'='*60}")
        print(f"V9: {account.name}")
        print(f"{'='*60}")
        
        sig = V9SignalGenerator(account.asset_class)
...
def run_v9():
    ...
    stock_acc = V9Account("Stocks V9", 10000, 'stocks')
    crypto_acc = V9Account("Crypto V9", 10000, 'crypto')
    forex_acc = V9Account("Forex V9", 10000, 'forex')
    options_acc = V9Account("Options V9", 10000, 'options')
    ...
    print("\n" + "="*80)
    print("V9 - 12 MONTH ENHANCED PERFORMANCE RESULTS")
    print("="*80)
    print(f"Period: {start.date()} to {end.date()} (12 months)")
    print("\nV9 ENHANCEMENTS:")
    print("  - Stocks: More positions (4), lower conf threshold, tighter trails")
    print("  - Crypto: Moderate approach with trend confirmation")
    print("  - Forex: Higher frequency trading, daily opportunities")
    print("  - Options: Proven covered call strategy")
    print("="*80)
    ...
    for name, s in results.items():
        if s:
            status = "PROFITABLE" if s['return_pct'] > 0 else ("BREAKEVEN" if s['return_pct'] == 0 else "LOSS")
            if s['return_pct'] > 0:
                profitable += 1
    ...
    print(f"  V9 (12mo): {total_ret:+.2f}% | {total_trades:3} trades | {profitable}/4 profitable")
    
    with open('backtesting/experiment_results_v9.json', 'w') as f:
        json.dump({
            'version': 9,
            'period_months': 12,
            ...
            'goal_met': profitable >= 3,
            'comparison': {
                'v8_6mo': {'return': 2.88, 'trades': 63, 'profitable': 3},
                'v9_12mo': {'return': round(total_ret, 2), 'trades': total_trades, 'profitable': profitable}
            }
        }, f, indent=2, default=str)
    
    print(f"\nSaved: backtesting/experiment_results_v9.json")
