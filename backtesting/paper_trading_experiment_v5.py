"""
Paper Trading Experiment V5 - PROFITABLE MULTI-ASSET STRATEGY
==============================================================
Based on V2's proven signal generation with:
1. Options trading via covered call simulation
2. More lenient trend filter for mean-reversion signals
3. Optimized per-asset parameters
4. Better risk/reward targeting 3:1 minimum
"""
...
class V5SignalGenerator:
    """V5 signal generator - V2 proven logic with enhancements"""
    
    def __init__(self, asset_class='stocks'):
        self.asset_class = asset_class
        self.thresholds = {
            'stocks': {'conf': 0.65, 'rsi_buy': 32, 'rsi_sell': 68},
            'crypto': {'conf': 0.68, 'rsi_buy': 28, 'rsi_sell': 72},
            'forex': {'conf': 0.62, 'rsi_buy': 30, 'rsi_sell': 70},
    ...
    def generate_signal(self, df, idx):
        ...
        t = self.thresholds[self.asset_class]
        row = df.iloc[idx]
        prev = df.iloc[idx - 1]
        ...
        if pd.notna(row['rsi']):
            if row['rsi'] < t['rsi_buy']:
                buy_pts += 3
                reasons.append(f"RSI {row['rsi']:.0f}")
            elif row['rsi'] > t['rsi_sell']:
                sell_pts += 3
                reasons.append(f"RSI {row['rsi']:.0f}")
            elif row['rsi'] < 38:
                buy_pts += 1
...
class V5Position:
    """Standard position with trailing stops"""
...
class V5Account:
    ...
    def __init__(self, name, balance, asset_class='stocks', max_dd=40):
        ...
        self.cfg = {
            'stocks': {'max_pos': 4, 'min_days': 2, 'risk': 1.5, 'max_pct': 0.22, 'stop_mult': 2.0, 'target_mult': 4.0},
            'crypto': {'max_pos': 2, 'min_days': 3, 'risk': 1.2, 'max_pct': 0.25, 'stop_mult': 2.5, 'target_mult': 5.5},
            'forex': {'max_pos': 3, 'min_days': 2, 'risk': 1.5, 'max_pct': 0.28, 'stop_mult': 1.5, 'target_mult': 3.0},
    ...
    def open_position(self, symbol, price, date, atr=None, is_option=False):
        ...
        if is_option:
            ...
            pos = V5Position(symbol, price, shares, date, stop, target, atr)
    ...
    def check_exits(self, symbol, high, low, current, date):
        ...
        if isinstance(pos, OptionsPosition):
            if pos.update(current, date):
                return self.close_position(symbol, current, date, 'expired/assigned')
            return None
...
class V5Engine:
    ...
    def _sim(self, symbol, start, end):
        ...
        cycle_len = 40 + np.random.randint(0, 20)
    ...
    def run(self, account, symbols, start, end, is_options=False):
        print(f"\n{'='*60}")
        print(f"V5: {account.name}")
        print(f"{'='*60}")
        
        sig = V5SignalGenerator(account.asset_class)
            ...
            for sym, df in data.items():
                ...
                if signal['signal'] == 'BUY' and sym not in account.positions:
                    if account.open_position(sym, row['Close'], date, signal.get('atr'), is_options):
                        print(f"    {date.date()}: BUY {sym} @ ${row['Close']:.2f} (conf: {signal['confidence']:.2f})")
...
def run_v5():
    ...
    stock_acc = V5Account("Stocks V5", 10000, 'stocks')
    crypto_acc = V5Account("Crypto V5", 10000, 'crypto')
    forex_acc = V5Account("Forex V5", 10000, 'forex')
    options_acc = V5Account("Options V5", 10000, 'options')
    ...
    print("\n" + "="*80)
    print("V5 PROFITABLE MULTI-ASSET RESULTS")
    print("="*80)
    print(f"Period: {start.date()} to {end.date()}")
    print("\nV5 FEATURES:")
    print("  - V2's proven signal logic")
    print("  - Options trading (covered call simulation)")
    print("  - Relaxed trend filter for strong signals")
    print("  - 3:1+ risk/reward targets")
    print("  - Asset-optimized parameters")
    print("="*80)
    ...
    print(f"{'='*80}")
    print("  V1:  -13.93% | 202 trades | 3 assets")
    print("  V2:   -1.82% |  32 trades | 3 assets")
    print("  V3:  -26.66% |  28 trades | 3 assets")
    print("  V4:   -0.04% |   2 trades | 4 assets")
    print(f"  V5:  {total_ret:+.2f}% | {total_trades:3} trades | 4 assets | {profitable}/4 profitable")
    
    with open('backtesting/experiment_results_v5.json', 'w') as f:
        json.dump({
            'version': 5,
            'run_date': datetime.now().isoformat(),
            'results': results,
            'profitable_count': profitable,
            'goal_met': profitable == 4,
            'comparison': {
                'v1': {'return': -13.93, 'trades': 202, 'assets': 3},
                'v2': {'return': -1.82, 'trades': 32, 'assets': 3},
                'v3': {'return': -26.66, 'trades': 28, 'assets': 3},
                'v4': {'return': -0.04, 'trades': 2, 'assets': 4},
                'v5': {'return': round(total_ret, 2), 'trades': total_trades, 'assets': 4, 'profitable': profitable}
            }
        }, f, indent=2, default=str)
    
    print(f"\nSaved: backtesting/experiment_results_v5.json")
    
    if profitable == 4:
        print("\n*** GOAL ACHIEVED: All 4 asset classes are profitable! ***")
    else:
