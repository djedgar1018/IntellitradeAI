"""
Paper Trading Experiment V6 - OPTIMIZED FOR PROFITABILITY
==========================================================
V5 showed options profitable. V6 optimizes remaining assets:
1. Stocks: Tighter stops, better entry timing
2. Crypto: More conservative, only strongest signals
3. Forex: Wider targets, better trend following
4. Options: Keep V5 settings (already profitable)
"""
...
class V6SignalGenerator:
    """V6 signal generator - optimized per asset"""
    
    def __init__(self, asset_class='stocks'):
        self.asset_class = asset_class
        self.params = {
            'stocks': {'conf': 0.70, 'rsi_buy': 28, 'rsi_sell': 72, 'mom_thresh': 3},
            'crypto': {'conf': 0.78, 'rsi_buy': 22, 'rsi_sell': 78, 'mom_thresh': 8},
            'forex': {'conf': 0.65, 'rsi_buy': 32, 'rsi_sell': 68, 'mom_thresh': 2},
            'options': {'conf': 0.65, 'rsi_buy': 32, 'rsi_sell': 68, 'mom_thresh': 3}
        }
    ...
    def generate_signal(self, df, idx):
        ...
        strong_momentum_up = pd.notna(row['mom_pct']) and row['mom_pct'] > p['mom_thresh']
        strong_momentum_down = pd.notna(row['mom_pct']) and row['mom_pct'] < -p['mom_thresh']
...
class V6Account:
    ...
    def __init__(self, name, balance, asset_class='stocks', max_dd=40):
        ...
        self.cfg = {
            ...
            'crypto': {'max_pos': 2, 'min_days': 4, 'risk': 0.8, 'max_pct': 0.20, 'stop_mult': 2.5, 'target_mult': 6.0},
    ...
    def open_position(self, symbol, price, date, atr=None, is_option=False):
        ...
        if is_option:
            ...
            pos = V6Position(symbol, price, shares, date, stop, target, atr)
...
class V6Engine:
    ...
    def run(self, account, symbols, start, end, is_options=False):
        print(f"\n{'='*60}")
        print(f"V6: {account.name}")
        print(f"{'='*60}")
        
        sig = V6SignalGenerator(account.asset_class)
...
def run_v6():
    ...
    stock_acc = V6Account("Stocks V6", 10000, 'stocks')
    crypto_acc = V6Account("Crypto V6", 10000, 'crypto')
    forex_acc = V6Account("Forex V6", 10000, 'forex')
    options_acc = V6Account("Options V6", 10000, 'options')
    ...
    print("\nV6 OPTIMIZATIONS:")
    print("  - Stocks: Fewer positions, stricter entries, wider targets")
    print("  - Crypto: Only strongest signals, strict trend filter")
    print("  - Forex: Balanced approach, wider targets")
    ...
    print("  V5:   +0.43% |  96 trades | 4 assets | 1/4 profitable")
    print(f"  V6:  {total_ret:+.2f}% | {total_trades:3} trades | 4 assets | {profitable}/4 profitable")
    
    with open('backtesting/experiment_results_v6.json', 'w') as f:
        json.dump({
            'version': 6,
            ...
            'comparison': {
                'v1': {'return': -13.93, 'trades': 202},
                'v2': {'return': -1.82, 'trades': 32},
                'v5': {'return': 0.43, 'trades': 96, 'profitable': 1},
                'v6': {'return': round(total_ret, 2), 'trades': total_trades, 'profitable': profitable}
            }
        }, f, indent=2, default=str)
    
    print(f"\nSaved: backtesting/experiment_results_v6.json")
    
    if profitable == 4:
        print("\n*** GOAL ACHIEVED: All 4 asset classes are profitable! ***")
    elif profitable >= 3:
        print(f"\n*** CLOSE: {profitable}/4 asset classes profitable ***")
    else:
